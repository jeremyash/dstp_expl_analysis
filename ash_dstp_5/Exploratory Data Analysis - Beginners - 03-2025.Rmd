...
pct_lacess_hhnv15_hist <- ggplot() +
  ...
  # add a title
  labs(title = "Distribution of Percentage of Households with No Vehicle and Low Store Access")
...
Adding more detail to our exploratory analysis, we will now look at whether the distribution of `PCT_LACCESS_HHNV15` differs between metro and non-metro areas. It's not obvious to me what the differences are by looking at the histogram but the differences are clearer looking at the boxplots. Here we can see that households in non-metro areas are more likley to have no vehicle and lower access to stores, as you might expect.
...
I live in North Carolina, so I'm interested in exploring child poverty rate patterns across the state. 

```{r bonus, warning=FALSE, message = FALSE}
